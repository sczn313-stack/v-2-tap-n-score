...
assert.match(targetJs, /SCZN3SECSessionTimeline\.preservedRecords\(\{ sessions: localSessions \}\)/, "live timeline consumes only locally held session identities");
assert.doesNotMatch(targetJs, /fetch\(preservedSecEndpoint\s*,\s*\{\s*method:\s*"GET"/s, "live timeline never enumerates the global backend SEC collection");
