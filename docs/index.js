...
(() => {
  ...
  function renderSec() {
    ...
    if (els.secWindage) {
      els.secWindage.textContent = `${sec.windageDir} ${toFixed2(sec.windageClicks)} clicks`;
    }

    if (els.secElevation) {
      els.secElevation.textContent = `${sec.elevationDir} ${toFixed2(sec.elevationClicks)} clicks`;
    }
  }
  ...
  function onPhotoChosen(file) {
    ...
    reader.onload = () => {
      ...
      try {
        localStorage.setItem(STORAGE_KEYS.LAST_IMAGE, dataUrl);
      } catch (_) {}

      if (els.targetImg) {
        els.targetImg.onload = () => {
          els.targetImg.onload = null;
  ...
  function saveSEC() {
    ...
    img.onload = () => {
      ...
      y += 60;
      ctx.fillText(`Windage: ${state.secData.windageDir} ${toFixed2(state.secData.windageClicks)} clicks`, pad + 30, y);

      y += 60;
      ctx.fillText(`Elevation: ${state.secData.elevationDir} ${toFixed2(state.secData.elevationClicks)} clicks`, pad + 30, y);

      y += 60;
      ctx.fillText(`Group Size: ${toFixed2(state.secData.groupSizeInches)} in`, pad + 30, y);

      y += 60;
      ctx.fillText(`Offset X/Y: ${toFixed2(state.secData.dxInches)} in / ${toFixed2(state.secData.dyInches)} in`, pad + 30, y);

      y += 60;
      ctx.fillText(`Range: ${toFixed2(state.secData.rangeYds)} yds • ${state.secData.dialUnit} • ${toFixed2(state.secData.clickValue)} / click`, pad + 30, y);
