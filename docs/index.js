...
(() => {
  ...
  function renderDots() {
    ...
    if (poi) {
      const d = document.createElement("div");
      d.className = "shotDot poiDot";
      d.style.left = `${poi.x * 100}%`;
      d.style.top = `${poi.y * 100}%`;
      d.setAttribute("title", "POI");
      els.dotsLayer.appendChild(d);
    }
  }
  ...
  async function setWorkingImage(src) {
    ...
    return await new Promise((resolve) => {
      ...
      if (els.targetImg.complete && els.targetImg.naturalWidth > 0) {
        els.targetImg.onload();
      }
    });
  }
  ...
  function buildSummary() {
    ...
    const dx = round2(poi.x - state.aim.x);
    const dy = round2(poi.y - state.aim.y);
  ...
  function drawPoiMarker(ctx, x, y) {
    ctx.beginPath();
  ...
  function exportVisibleResult() {
    ...
    loadImage(els.targetImg.src)
      .then((img) => {
        ...
        if (summary.poi) {
          drawPoiMarker(ctx, summary.poi.x * width, summary.poi.y * height);
        }
